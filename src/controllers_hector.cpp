		...
		if(trigger==1 & (int)vive.ctrl_right.buttons.trigger == 0)
		  {
			...
			z_tar = waypoint.pose.position.z;
		...
		if(trigger==0 & (int)vive.ctrl_right.buttons.trigger == 1)
		  {
			...
			z_ori = vive.ctrl_right.pose.position.z;
		...
		if((int)vive.ctrl_right.buttons.trigger == 1)
		  {
			...
			float z_diff = vive.ctrl_right.pose.position.z - z_ori;

			base_motion.linear.x = x_diff * kp_lin;
			base_motion.linear.y = y_diff * kp_lin;
			base_motion.linear.z = z_diff * kp_lin;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_vive = nh.subscribe("openvr_headset_ros/vive", 1, &Vive_Listener::callback, &vive_data);
    ...
    velocity_controller.get_state.request.model_name = "quadrotor";
...
//    Way_point_controller.readmodel((pkglocalpath + "/../../../.gazebo/models/controller/model.sdf").c_str()); // readmodel req.s const char* currently // "/home/USERNAME/.gazebo/models/controller/model.sdf"
    Way_point_controller.readmodel((pkglocalpath + "/models/controller/model.sdf").c_str()); // readmodel req.s const char* currently // "/home/USERNAME/.gazebo/models/controller/model.sdf"
    ...
    Way_point_controller.kp_x = 1;
    Way_point_controller.kd_x = 0.3;
    Way_point_controller.kp_z = 2;
    Way_point_controller.kd_z = 0.4;
    ...
    ThrowTo.readmodel((pkglocalpath + "/models/controller/model.sdf").c_str()); // readmodel req.s const char* currently // "/home/USERNAME/.gazebo/models/controller/model.sdf"
  ...
  if(vive_previ.ctrl_right.buttons.system == 0 & vive_data.vive.ctrl_right.buttons.system == 1)
	{
		...
		switch(controller_switch) {
		   ...
		   case 3  :
			...
			Way_point_controller.client_get.call(get_state);
